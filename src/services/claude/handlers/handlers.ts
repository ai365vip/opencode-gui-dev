...
export async function handleInit(
    ...
    // 获取模型设置
    let modelSetting = configService.getValue<string>('claudix.selectedModel') || 'default';

    // UI/会话层不使用 "default" 作为具体模型（避免无法在模型列表中命中、无法显示选中态）
    if (modelSetting === 'default') {
        modelSetting = 'claude-sonnet-4-5-20250929';
    }

    // 迁移旧的模型ID到新的完整ID
    const oldToNewModelMap: Record<string, string> = {
        'claude-sonnet-4-5': 'claude-sonnet-4-5-20250929',
        'claude-opus-4-1': 'claude-opus-4-1-20250805'
    };

    if (oldToNewModelMap[modelSetting]) {
        const newModelId = oldToNewModelMap[modelSetting];
        logService.info(`[handleInit] 迁移模型ID: ${modelSetting} → ${newModelId}`);
        modelSetting = newModelId;
        // 更新配置以便下次直接使用新ID
        try {
            await configService.updateValue('claudix.selectedModel', newModelId);
        } catch (error) {
            logService.warn(`[handleInit] 无法更新模型配置: ${error}`);
        }
    }
...
let cachedClaudeState: any | null = null;
let cachedClaudeStateAt = 0;
let cachedClaudeStatePromise: Promise<any> | null = null;
...
async function loadConfig(context: HandlerContext): Promise<any> {
    const now = Date.now();
    if (cachedClaudeState && now - cachedClaudeStateAt < CONFIG_CACHE_TTL_MS) {
        return cachedClaudeState;
    }

    if (cachedClaudeStatePromise) {
        return cachedClaudeStatePromise;
    }

    cachedClaudeStatePromise = (async () => {
        ...
        try {
            ...
            // 防御式：单个接口失败不应导致整个 get_claude_state 失败
            const slashCommands = typeof (query as any).supportedCommands === 'function'
                ? await (query as any).supportedCommands().catch(() => [])
                : [];
            const models = typeof (query as any).supportedModels === 'function'
                ? await (query as any).supportedModels().catch(() => [])
                : [];
            const accountInfo = typeof (query as any).accountInfo === 'function'
                ? await (query as any).accountInfo().catch(() => null)
                : null;

            const config = { slashCommands, models, accountInfo };

            cachedClaudeState = config;
            cachedClaudeStateAt = Date.now();

            logService.info(`  - Config: [${JSON.stringify(config)}]`);
            return config;
        } catch (error) {
            logService.error(`[loadConfig] Failed to load Claude state: ${error}`);
            ...
            // 缓存短时间兜底，避免持续重试打满资源
            cachedClaudeState = fallback;
            cachedClaudeStateAt = Date.now();
            return fallback;
        } finally {
            try { inputStream.done(); } catch { }
            try { await query?.return?.(); } catch { }
        }
    })();

    try {
        return await cachedClaudeStatePromise;
    } finally {
        cachedClaudeStatePromise = null;
    }
}
...
async function getMcpServers(
    ...
    try {
        // 通过 agentService 获取 channel
        const channel = agentService.getChannel(channelId);

        if (channel?.query) {
            // 使用 SDK v0.2.x 的 mcpServerStatus() 方法
            const mcpServers = await channel.query.mcpServerStatus();
            logService.info(`[MCP] 获取到 ${mcpServers.length} 个 MCP 服务器状态`);
            return {
        ...
        logService.warn('[MCP] Channel 或 query 不存在');
        return {
            type: "get_mcp_servers_response",
            mcpServers: []
        };
...
async function findFiles(pattern: string | undefined, context: HandlerContext): Promise<ListFilesResponse["files"]> {
    ...
    try {
        ...
        // 特殊处理：如果是通用搜索模式（如 ** 或 *），收集顶级目录
        if (pattern === '**' || pattern === '*') {
            const { workspaceService } = context;
            const cwd = workspaceService.getDefaultWorkspaceFolder()?.uri.fsPath || process.cwd();
            const topLevelDirs = await collectChildDirs(cwd, '', context);
            topLevelDirs.sort((a, b) => a.name.localeCompare(b.name))
            dirResults.push(...topLevelDirs);
        }
...
async function findFilesWithWorkspaceSearch(
    ...
    // 修改为开头匹配：如果没有通配符，搜索以pattern开头的文件
    ...
    const fileResults: ListFilesResponse["files"] = [];
    for (const uri of uris) {
        const fsPath = uri.fsPath;
        let type: "file" | "directory" = "file";
        let mtime = new Date(0); // 默认最旧时间
        try {
        ...
        fileResults.push({
            ...
            type,
            mtime: mtime.getTime() // 保存为时间戳便于排序
        });
    }

    // 如果是简单搜索（非通配符），过滤结果确保文件名开头匹配
    let filteredFileResults = fileResults;
    if (!pattern.includes("*") && !pattern.includes("?") && pattern.trim()) {
    ...
    // 由文件结果推导父目录
    const derived = deriveParentDirsFromFiles(filteredFileResults, context, anchorInfo.lastToken);
    dirResults.push(...derived);
    ...
    const merged: ListFilesResponse["files"] = [];
    if (anchorInfo.anchorAbs) {
        merged.push(...dirMerged, ...filteredFileResults);
    } else {
        merged.push(...filteredFileResults, ...dirMerged);
    }
...
function prioritizeRipgrepResults(
    ...
    const scored = cleaned.map((raw, index) => {
        ...
        if (normalizedPattern.length > 0) {
            // 改为开头匹配
            const pathStartsWith = lowerRelative.startsWith(normalizedPattern);
            const nameStartsWith = lowerName.startsWith(normalizedPattern);
            matched = pathStartsWith || nameStartsWith;
            if (pathStartsWith) score = Math.min(score, 0); // 路径开头匹配优先级最高
            if (nameStartsWith) score = Math.min(score, 1); // 文件名开头匹配次之
        } else {
            score = index;
        }
