...
const filePath = computed(() => {
  return props.toolUse?.input?.file_path || props.toolUse?.input?.path || props.toolUse?.input?.filePath || '';
});
...
const replaceAll = computed(() => {
  return props.toolUse?.input?.replace_all ?? props.toolUse?.input?.replaceAll;
});
...
watch(
  ...
  () => {
    ...
    // 如果有 input,生成临时 diff(权限请求阶段或实时对话执行完成后保留)
      ...
      const oldString = props.toolUse?.input?.old_string ?? props.toolUse?.input?.oldString;
      const newString = props.toolUse?.input?.new_string ?? props.toolUse?.input?.newString;

      if (typeof oldString === 'string' && typeof newString === 'string') {
        structuredPatch.value = generatePatchFromInput(oldString, newString);
      }
    }
  },
...
const isPermissionRequest = computed(() => {
  ...
  const oldString = props.toolUse?.input?.old_string ?? props.toolUse?.input?.oldString;
  const newString = props.toolUse?.input?.new_string ?? props.toolUse?.input?.newString;
  const hasInputDiff = typeof oldString === 'string' && typeof newString === 'string';
...
function getLineNumber(patch: any, lineIndex: string | number): string {
