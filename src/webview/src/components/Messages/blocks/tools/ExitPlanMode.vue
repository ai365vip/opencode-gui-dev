...
function sanitizeHtml(html: string): string {
  ...
  // DOMParser 在 WebView 环境可用；兜底处理非浏览器环境（测试/SSR）
...
// 使用 marked 渲染 Markdown，并做基础 sanitization（避免 v-html 注入风险）
const renderedPlan = computed(() => {
  if (!plan.value) return '';
  const html = marked(plan.value) as unknown as string;
